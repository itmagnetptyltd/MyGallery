...
13. [When something does not work](#13-when-something-does-not-work)
    ...
    - 13.5 [MyGallery will not start: "Python was not found"](#135-mygallery-will-not-start-python-was-not-found)
    - 13.6 [MyGallery will not start: "Address already in use"](#136-mygallery-will-not-start-address-already-in-use)
    - 13.7 [The browser did not open](#137-the-browser-did-not-open)
...
MyGallery needs the following before it will start.
...
**Result:** Python 3.12 or later is installed on your PC.
...
Your first Photo takes four steps, from opening MyGallery to seeing its
Thumbnail.
...
### 6.2 What a Thumbnail card shows
...
you uploaded, under the filename you uploaded it with.
...
Backing up your Gallery means copying your Photos out of `Pictures\MyGallery` to
somewhere else, such as an external drive or another folder.

**To back up your Gallery:**

- In File Explorer, copy the files in `Pictures\MyGallery` to another location.
...
The format is judged by the file's content, not its extension, so a file
renamed to `.jpg` is still refused where its content is not a JPEG. HEIC and
...
| Symptom | See |
| --- | --- |
| MyGallery will not start, on its first run | [13.8 Windows blocked MyGallery from creating its Gallery folder](#138-windows-blocked-mygallery-from-creating-its-gallery-folder) |
| "Python was not found" | [13.5 MyGallery will not start: "Python was not found"](#135-mygallery-will-not-start-python-was-not-found) |
| "Address already in use" | [13.6 MyGallery will not start: "Address already in use"](#136-mygallery-will-not-start-address-already-in-use) |
| The browser did not open | [13.7 The browser did not open](#137-the-browser-did-not-open) |
| "The Gallery could not be read." | [13.9 The Gallery says it could not be read](#139-the-gallery-says-it-could-not-be-read) |
| Some of the files you chose are not in the Gallery | [13.2 Some of the files you chose are not in the Gallery](#132-some-of-the-files-you-chose-are-not-in-the-gallery) |
| A file was refused as not a supported image type | [13.3 A file was refused as not a supported image type](#133-a-file-was-refused-as-not-a-supported-image-type) |
| A file was refused as too large | [13.4 A file was refused as too large](#134-a-file-was-refused-as-too-large) |
...
### 13.2 Some of the files you chose are not in the Gallery

Where one or more files in an Upload fail, the files that succeeded are still
added as Photos.

**Before you begin**

- See [13.1 What happens when part of an Upload fails](#131-what-happens-when-part-of-an-upload-fails).

**To find out which files failed, and why:**

- On the Gallery page, read the name and reason shown beside each failed file,
  as in "notes.txt: not a supported image type".

**Result:** You know which files were not added, and why.
...
### 13.3 A file was refused as not a supported image type

A file is refused with the reason "not a supported image type" where it is not
in an accepted image format, as in "notes.txt: not a supported image type".

![An upload refusal reading "notes.txt: not a supported image type"](Images/08-upload-refused.png)

This happens where the file's content is not JPEG, PNG, GIF or WebP, including a
file renamed to carry one of those extensions, and including HEIC and camera RAW
files. See [12.1 Which files become a Photo](#121-which-files-become-a-photo).

**To add the Photo:**

- Upload the file again, in an accepted image format.

**Result:** The file is accepted as a Photo.
...
### 13.4 A file was refused as too large
...
### 13.5 MyGallery will not start: "Python was not found"
...
### 13.6 MyGallery will not start: "Address already in use"

MyGallery shows "Address already in use" where it is already running.

**To fix this:**

- Look for the black window MyGallery opened, or go to `http://127.0.0.1:8765`
  in your web browser.

**Result:** Your Gallery is shown, either in the window already open or in your
browser.
...
### 13.9 The Gallery says it could not be read

MyGallery shows "The Gallery could not be read." where it cannot open your
Gallery.

The one known cause is Controlled folder access blocking MyGallery from
creating its Gallery folder. See [13.8 Windows blocked MyGallery from creating its Gallery folder](#138-windows-blocked-mygallery-from-creating-its-gallery-folder).
...
These controls are named throughout this manual, but are not concepts of their
own: **Upload photos**, **Choose files**, **Description (optional)**, **Save
description**, **Keep it**.

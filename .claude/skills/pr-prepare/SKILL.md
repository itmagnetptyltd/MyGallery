...
node .claude/itm-sdlc/scripts/close-slice.js --project . REQ-...
```

Report failures rather than pushing and letting the gates find them.

If `close-slice` still lists OPEN ids among the ones this PR claims to
cover, **stop**. Those ids are not verified. Finish `/tdd` or write the
missing `@covers`, then run `/close-slice` again. A pull request that leaves
the slice In progress is how the dashboard gets stuck.

If this is a `chore`, skip `close-slice`.
...
The tests, and how they are annotated **per required belt** (unit, e2e if B, HTTP if C). If a criterion was verified by hand,
...
- Tests annotate those requirements at their **current** version in **every `belts:` letter**
...
- **The one permitted `.brain/` change:** `status`, `version`, `verified_by` and
  ...
  should have been written by `/tdd` and `/close-slice`, not by hand — if

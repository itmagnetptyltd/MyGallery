...
| Check | Answers |
|---|---|
| `validate-requirements` | Is the record internally consistent — ids unique, dependencies real, no cycles, nothing `agreed` with open ambiguities, nothing `verified` without `verified_by` in every required belt? |
| `check-traceability` | Orphans in both directions, plus `missing-belt` when a required A/B/C file is absent. `--strict` matches G3 on the PR (exit 1). |
...
**By default, stop there.** Reporting is this skill's job. After `/tdd`,
the way to record is `/close-slice`, not this command — `/tdd` already
calls it.

### Recording it

**Only when the developer asked** — `/verifyReq --record`, `/close-slice`,
or they said in this conversation to record the transition — drop `--dry-run`:

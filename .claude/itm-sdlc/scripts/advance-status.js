...
Usage: node advance-status.js --to <status> REQ-ID [REQ-ID ...]
  ...
  A mixed list is not an error. Ids already at the target are skipped.
  --to in_progress also skips ids already verified or signed_off, so a
  slice that is part-done does not refuse the ids that still need to move.
...
function setScalar(lines, block, key, value) {
  ...
  lines[at] =
    `${" ".repeat(block.fieldIndent)}${key}: ${value}${comment ? comment[1] : ""}`;
  return true;
}
...
function setVerifiedBy(lines, block, files, projectDir) {
  ...
  for (let i = at + 1; i < block.end; i += 1) {
    if (/^\s*-\s/.test(lines[i]) && indentOf(lines[i]) > block.fieldIndent)
      last = i;
    else break;
  }
  ...
  for (const adapter of adapters) {
    for (const file of testFiles(projectDir, adapter)) {
      ...
      for (const found of scanFile(file, adapter)) {
        if (found.id === id && found.version === version)
...
function gate(requirement, to, projectDir) {
  ...
  if (fromIndex === -1) {
    return {
      ok: false,
      reason: `current status '${from}' is not one of: ${STATUS_ORDER.join(", ")}`,
    };
  }
  if (from === to) {
  ...
  if (toIndex < fromIndex) {
    // A mixed slice (some verified, some still agreed) must not deadlock
    // /tdd: skip ids already past the target instead of refusing the batch.
    if (
      to === "in_progress" &&
      (from === "verified" || from === "signed_off")
    ) {
      return {
        ok: true,
        note: `already at '${from}' — skipped, not moved back`,
      };
    }
    return {
      ok: false,
      reason: `already at '${from}'. The lifecycle is forward-only (CONVENTIONS.md §6); go back by incrementing version instead.`,
    };
  }
  if (toIndex !== fromIndex + 1) {
    return {
      ok: false,
      reason: `at '${from}', so '${to}' would skip ${STATUS_ORDER.slice(fromIndex + 1, toIndex).join(", ")}. Transitions may not skip a step.`,
    };
  }

  if (to === "agreed") {
    const open = Array.isArray(requirement.ambiguities)
  ...
  if (to === "verified") {
    ...
    const { files, scanned, adapters } = coveringTests(
      projectDir,
      requirement.id,
      version,
    );
    ...
    if (adapters.length === 0) {
      return {
        ok: false,
        reason:
          "no language adapter matched this project, so no test file was scanned. Nothing can be verified against evidence that was never read.",
      };
    }
    if (files.length === 0) {
      return {
    ...
    const belts = require("./lib/belts");
    const { detectAdapters: detectForBelts } = require("./lib/adapters");
    const missing = belts.missingBelts(
      requirement,
      files,
      projectDir,
      detectForBelts(projectDir),
    );
    if (missing.length > 0) {
      return {
        ok: false,
        reason: `missing belt ${missing.join(", ")} @covers at the current version. Required belts: [${belts.requiredBelts(requirement).join(", ")}]. An e2e file does not satisfy A; a unit file does not satisfy B or C.`,
      };
    }
...
function advance({
  ...
  const { documents, missingPatterns } = loadRequirementFiles(
    patterns,
    projectDir,
  );
  if (missingPatterns.length > 0) {
    throw new Error(
      `no requirement file matched ${missingPatterns.join(", ")} under ${projectDir}`,
    );
  }
  ...
  if (unreadable.length > 0) {
    throw new Error(
      `cannot read the requirement record: ${unreadable.map((d) => `${d.file}: ${d.error}`).join("; ")}`,
    );
  }
  ...
  for (const id of ids) {
    ...
    moved.push({
      ...
      verifiedBy: (verdict.verifiedBy ?? []).map((f) =>
        toPosix(path.relative(projectDir, f)),
      ),
    });
  }
  ...
  if (!dryRun) {
    for (const [file, items] of edits) {
      ...
      for (const item of items) {
        ...
        if (!block) {
          throw new Error(
            `${item.id} parsed from ${file} but its block could not be located for editing. The file was not modified.`,
          );
        }
        if (!setScalar(lines, block, "status", to)) {
          throw new Error(
            `${item.id} has no status: line in ${file}. The file was not modified.`,
          );
        }
        if (item.verifiedBy) {
...
function parseArgs(argv) {
  ...
  if (options.ids.length === 0)
    throw new Error("name at least one requirement id");
...
function report(result) {
  const out = [""];
  out.push(
    `itm-sdlc | status${result.dryRun ? "  (DRY RUN - nothing was written)" : ""}`,
  );
  out.push("");
  ...
  for (const item of result.moved) {
    ...
    for (const file of item.verifiedBy)
      out.push(`          verified_by  ${file}`);
  }
  for (const item of result.unchanged)
    out.push(`  --    ${item.id}  ${item.note}`);
  ...
  out.push("  " + "-".repeat(66));
  out.push(
    `  ${result.moved.length} moved, ${result.unchanged.length} already there, ${result.refused.length} refused`,
  );
  out.push(
    result.ok
      ? `  ${result.dryRun ? "DRY RUN - no changes made" : "OK"}`
      : "  REFUSED - the record was not changed for those requirements.",
  );
  out.push("");

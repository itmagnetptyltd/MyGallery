 ...
 *   E  missing-belt           a required testing belt (A/B/C) has no current @covers file
 *
...
const { requiredBelts, classifyRelativePath } = require("./lib/belts");
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--requirements":
        i += 1;
        if (argv[i] === undefined)
          throw new Error("--requirements requires a glob");
  ...
  if (options.requirements.length === 0)
    options.requirements = [...DEFAULT_PATTERNS];
  return options;
}
...
const orphan = (code, message, context) =>
  finding("orphan", code, message, context);
const warning = (code, message, context) =>
  finding("warning", code, message, context);
  ...
  for (const { requirement, file } of entries) {
    ...
    findings.push(
      orphan(
        ...
        `status is '${requirement.status}' but no test annotates it. Either a test is missing, or the status is ahead of reality.`,
        {
          file,
          id: requirement.id,
        },
...
/** E: a required belt (A/B/C) has no current-version @covers in that belt's globs. */
function checkMissingBelts(entries, annotationsById, adapters, projectDir) {
  const findings = [];

  for (const { requirement, file } of entries) {
    if (!statusAtLeast(requirement.status, COVERAGE_REQUIRED_FROM)) continue;
    const annotations = (annotationsById.get(requirement.id) ?? []).filter(
      (annotation) => annotation.version === requirement.version,
    );
    if (annotations.length === 0) continue;

    const present = new Set();
    for (const annotation of annotations) {
      const rel = relative(projectDir, annotation.file);
      const belt = classifyRelativePath(rel, adapters);
      if (belt) present.add(belt);
    }

    for (const belt of requiredBelts(requirement)) {
      if (present.has(belt)) continue;
      findings.push(
        orphan(
          "missing-belt",
          `requires belt ${belt} but no current @covers is in a belt-${belt} test file. Required: [${requiredBelts(requirement).join(", ")}].`,
          { file, id: requirement.id },
        ),
      );
    }
  }
  ...
  for (const annotation of annotations) {
    ...
    if (!requirement) {
      findings.push(
        orphan(
          "unknown-requirement",
          `annotates ${annotation.id}, which does not exist in any requirement file. The requirement was renamed, deleted, or never written.`,
          {
    ...
    if (annotation.version === null) {
      findings.push(
        orphan(
          "missing-version",
          `annotates ${annotation.id} with no @v. The version is mandatory (CONVENTIONS.md §5); write @covers ${annotation.id}@v${requirement.version}.`,
          {
            file: annotation.file,
            line: annotation.line,
            id: annotation.id,
    ...
    if (annotation.version !== requirement.version) {
      findings.push(
        orphan(
          "stale-version",
          `annotates ${annotation.id}@v${annotation.version} but the current version is v${requirement.version}. The test was written against text that has since changed and must be re-read before the pin is moved.`,
          {
            file: annotation.file,
            line: annotation.line,
            id: annotation.id,
...
function check(options, adapterList = undefined) {
  ...
  const { documents } = loadRequirementFiles(
  ...
  const { annotations, scannedFiles } = collectAnnotations(
    options.project,
    adapters,
  );
  ...
  for (const annotation of annotations) {
    if (!annotationsById.has(annotation.id))
      annotationsById.set(annotation.id, []);
  ...
  if (adapters.length === 0) {
    findings.push(
      warning(
        "no-adapters",
        `no language adapter matched ${options.project}. No test file was scanned, so this result proves nothing.`,
      ),
    );
  }
  ...
  if (entries.length === 0) {
    findings.push(
      warning(
        "no-requirements",
        `no requirements found under ${options.requirements.join(", ")}. Nothing to trace.`,
      ),
    );
  }
  ...
  findings.push(
    ...checkMissingBelts(entries, annotationsById, adapters, options.project),
  );
  ...
  const covered = tracked.filter(({ requirement }) =>
    annotationsById.has(requirement.id),
  );
...
const CLASS_LABELS = {
  ...
  "unknown-requirement":
    "B  annotations naming a requirement that does not exist",
  "stale-version": "C  annotations pinned to a superseded version",
  "missing-version": "D  annotations with no @v version",
  "missing-belt": "E  required testing belt with no @covers file",
};
...
function printHumanReport(result) {
  ...
  lines.push(
    `  scanned    ${result.scannedFileCount} test file(s), ${result.annotationCount} annotation(s)`,
  );
  lines.push(
    `  coverage   ${result.coverage.covered}/${result.coverage.requiringCoverage} requirement(s) at '${COVERAGE_REQUIRED_FROM}' or beyond are annotated`,
  );
  lines.push("");
  ...
  lines.push(
    `  ${result.orphanCount} orphan(s), ${result.warningCount} warning(s)  |  mode: ${result.mode}`,
  );
  ...
  if (result.orphanCount === 0) {
    ...
    lines.push(
      "  This gate blocks nothing until its false-positive rate has been measured.",
    );
  } else {
    lines.push("  FAIL");
  }

  ...
  for (const entry of entries) {
    ...
    if (previous) {
      findings.push(
        error(
          "duplicate-id",
          `${id} is already declared in ${previous.file}. Requirement ids are globally unique and are never reused (CONVENTIONS.md §1).`,
          {
            file: entry.file,
            id,
          },
  ...
  for (const { requirement, file } of entries) {
    ...
    if (fromId && fromId !== module) {
      findings.push(
        error(
          "module-mismatch",
          `module is '${module}' but the id declares '${fromId}'. One of the two is wrong; the id is frozen, so fix the module field.`,
          {
            file,
            id,
          },
    ...
    for (const target of dependencies) {
      if (!index.has(target)) {
        findings.push(
          error(
            "unknown-dependency",
            `depends_on references ${target}, which does not exist in any scanned requirement file.`,
            {
              file,
...
function checkNoDependencyCycles(entries, index) {
  ...
  const dependenciesOf = (id) => {
    ...
    return Array.isArray(dependencies)
      ? dependencies.filter((d) => index.has(d))
      : [];
  };
  ...
  const record = (cycle) => {
    ...
    findings.push(
      error(
        "dependency-cycle",
        `dependency cycle: ${[...cycle, cycle[0]].join(" -> ")}. A requirement cannot transitively depend on itself.`,
        {
  ...
  for (const { requirement, file } of entries) {
    ...
    if (open.length > 0) {
      ...
      findings.push(
        error(
          "open-ambiguity",
          `status is '${status}' but ${open.length} ${plural} still open. An ambiguity is resolved by the client answering it, never by the toolkit choosing a reading (CONVENTIONS.md §6).`,
          {
            file,
            id: id ?? null,
          },
    ...
    if (evidence.length === 0) {
      findings.push(
        error(
          ...
          `status is '${status}' but verified_by is empty. A requirement is verified by a test that annotates it, or it is not verified.`,
          {
            file,
            id: id ?? null,
          },
...
/**
 * CONVENTIONS.md §6: verified_by must include a file for each required belt.
 * Path classification uses shipped adapters (not detect), so a listed e2e file
 * cannot stand in for A and a unit file cannot stand in for B or C.
 */
function checkVerifiedHasBelts(entries) {
  const { loadAdapters } = require("./lib/adapters");
  ...
  for (const { requirement, file } of entries) {
    ...
    const required = requiredBelts(requirement);
    for (const belt of required) {
      if (present.has(belt)) continue;
      findings.push(
        error(
          "missing-belt",
          `status is '${status}' but verified_by has no belt-${belt} file. Required: [${required.join(", ")}]. An e2e path does not satisfy A; a unit path does not satisfy B or C.`,
          { file, id: id ?? null },
        ),
      );
    }
...
function schemaFindings(documents, validateDocument) {
  ...
  for (const doc of documents) {
    ...
    for (const err of validateDocument.errors) {
      ...
      const subject = Number.isInteger(index)
      ...
      findings.push(
        error(
          "schema",
          `${field}: ${err.message}${err.params && Object.keys(err.params).length ? ` (${JSON.stringify(err.params)})` : ""}`,
          {
            file: doc.file,
...
function validate(options) {
  ...
  if (missingPatterns.length > 0) {
    findings.push(
      warning(
        "no-files",
        `no requirement files matched: ${missingPatterns.join(", ")}. Nothing was validated.`,
      ),
    );
  }
  ...
  const index = new Map();
  for (const entry of entries)
    if (!index.has(entry.requirement.id))
      index.set(entry.requirement.id, entry.requirement);
  ...
  findings.push(...checkVerifiedHasBelts(entries));
...
function printHumanReport(result, options) {
  ...
  for (const file of result.files) {
    ...
    for (const problem of problems) {
      lines.push(
        `          ${problem.level.toUpperCase()}  [${problem.code}]  ${problem.id ?? ""}`.trimEnd(),
      );
  ...
  for (const problem of unattached) {
    lines.push(
      `  ${problem.level.toUpperCase()}  [${problem.code}]  ${problem.message}`,
    );
  }

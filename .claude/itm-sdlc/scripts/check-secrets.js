...
const RULES = [
  ...
  },
  {
    ...
    pattern:
      /[A-Za-z0-9_]{0,24}(?:password|passwd|secret|api[_-]?key|apikey|access[_-]?token|auth[_-]?token|client[_-]?secret|private[_-]?key|token)["\x27`]?\s*[:=]\s*["'`]([^"'`\n]{8,})["'`]/i,
    // Anything that is plainly not a secret value.
    exempt:
      /^(?:\s*|\$\{.*\}|<[^>]*>|process\.env\..*|env\..*|null|undefined|true|false|x{3,}|\*{3,}|\.{3,}|changeme|password|secret|your[-_].*|my[-_].*|example.*|placeholder.*|todo.*|redacted.*|dummy.*|test|sample.*)$/i,
  },
];
...
function isExcluded(relative, excludes) {
  ...
  return excludes.some((ex) => {
    const exParts = ex.split(/[\\/]/).filter(Boolean);
    if (exParts.length === 0 || exParts.length > parts.length) return false;
    // Match at any depth so `bin` skips tests/Foo.Tests/bin/, not only ./bin.
    for (let i = 0; i <= parts.length - exParts.length; i += 1) {
      if (exParts.every((part, j) => parts[i + j] === part)) return true;
    }
    return false;
  });
}
...
function report(result) {
  ...
  if (result.findings.length === 0) {
    ...
    out.push("");
    out.push(
      "  This checks for literals matching known shapes. It does not do entropy",
    );
    ...
    out.push(
      "  A credential that reached a commit is compromised. Rotate it — removing",
    );
  ...
  out.push("  " + "-".repeat(66));
  out.push(
    `  ${result.blockingCount} blocking, ${result.findings.length - result.blockingCount} other  |  mode: ${result.mode}`,
  );
  out.push(
    result.ok
      ? "  PASS"
      : result.mode === "advisory"
        ? "  ADVISORY - reported, build not failed."
        : "  FAIL",
  );
  out.push("");
...
function main(argv) {
  ...
  if (options.command) {
    try {
      const status = runExternal(
        options.command,
        path.resolve(options.project),
      );
      return options.advisory
        ? EXIT_CLEAN
        : status === 0
          ? EXIT_CLEAN
          : EXIT_FOUND;
